...
try:
    from phantasy import fetch_data as _phantasy_fetch_data_orig
...
# Try importing EPICS library
try:
    #from epics_tool import _epics_caget, _epics_caput, _epics_fetch_data, _epics_ensure_set 
    from epics import caget as _epics_caget, caput as _epics_caput
...
if _phantasy_imported:    
    def _phantasy_fetch_data(pvlist: List[str],
                             time_span: Optional[float] = None,
                             verbose: Optional[bool] = None,
                             check_chopper_blocking: Optional[bool] = False,
                             test_mode=False, **kws):
                    
        """Fetch data with optional chopper blocking check."""
        kws.pop('data_opt', None)
        kws.pop('with_data', None)                   

        if check_chopper_blocking and not test_mode:
            try:
        ...
        while True:
            ave, raw = _phantasy_fetch_data_orig(
                pvlist, 
                time_span=time_span, 
                with_data=True, 
        ...
        display(raw)
        if len(raw) < 2:
            ave, raw = _phantasy_fetch_data(pvlist = pvlist,
                             time_span = time_span+1,
                             verbose = verbose,
                             check_chopper_blocking = check_chopper_blocking,
                             test_mode=test_mode, **kws)
        elif (raw.index[-1] - raw.index[0]).total_seconds() < 2:
            ave, raw = _phantasy_fetch_data(pvlist = pvlist,
                 time_span = time_span+1,
                 verbose = verbose,
                 check_chopper_blocking = check_chopper_blocking,
...
class construct_machineIO(AbstractMachineIO):
    ...
    def _fetch_data(self,
        ...
        ave, raw = _phantasy_fetch_data(pvlist = pvlist,
                                        time_span = time_span,
                                        verbose = verbose,
                                        check_chopper_blocking = self._check_chopper_blocking,
...
class Evaluator:
    def __init__(self,
        ...
        - input_tols: List or array of tolerances for each control PV.
        - output_RDs: Optional list of additional readback PVs.
        - ensure_set_kwargs: Optional keyword arguments for ensure_set.
        - fetch_data_kwargs: Optional keyword arguments for fetch_data.
        """
...
class Evaluator_wBPMQ(Evaluator):
    def __init__(self,
        ...
        if BPM_names is not None:
            self.raw2Q = raw2Q_processor(BPM_names=BPM_names,BPMQ_models=BPMQ_models)
