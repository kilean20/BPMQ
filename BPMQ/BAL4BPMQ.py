...
_xalpha,_xbeta,_xnemit = 0.0, 5.0, 0.2*1e-6
_yalpha,_ybeta,_ynemit = 0.0, 5.0, 0.2*1e-6
...
def noise2cs(
    ...
    weights = torch.tensor([2, 0.7, 0.4, 2, 0.7, 0.4], device=noise.device)
        ...
        def loss_fun(x):
            ...
            if PM_llB2 is not None:
                ...
                if fit_err:
                    ...
                    regloss_PM_err = 0.5*torch.mean(torch.abs(xPM_err) + torch.abs(yPM_err), dim=[0,1])
                else:
                    target = BPMQ_targets
                    xPM_target = PM_xrms_targets
                    yPM_target = PM_yrms_targets
                fitloss_PM = torch.mean(torch.abs(xPM_sim/xPM_target -1.0) / PM_xrms_tolerances
...
#                 is_beamloss = np.any(BPM_MAGs < 0.95*self.init_BPM_MAGs)
    ...
    def train_model(self, train_llB2=None,train_llBPMQ=None,train_llBPMQtol=None,train_llBPMQmodelerr=None,
                          xnemit_target=None,ynemit_target=None,fit_err=None,sample_model_err=None, bootstrap=None,
                          retrun_loss_ftn_4_debug=False):
        ...
        _ = self.model.cs_reconstruct(self.i_bpms, train_llB2, train_llBPMQ, train_llBPMQtol, train_llBPMQmodelerr,
                                  PM_i_monitors=self.i_pms, PM_llB2=self.train_llB2rms, 
                                  PM_xrms_targets=self.train_llxrms, PM_yrms_targets=self.train_llyrms, 
                                  PM_xrms_tolerances=self.train_llxrmstol, PM_yrms_tolerances=self.train_llyrmstol,
