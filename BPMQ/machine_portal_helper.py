...
try:
    ...
    print("MachinePortal functionality will be unavailable.")
    class MPelem():
        def __init__(self, name):
            self.name = name
        def convert(self, val, from_field, to_field):
            return val

    def get_MPelem_from_PVnames(names: List):
        elems = [MPelem(name) for name in names]
        return elems
