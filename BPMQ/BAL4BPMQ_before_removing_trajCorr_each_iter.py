...
def cs2noise(
    ...
    """
    Inverse of noise2cs: convert Courant-Snyder parameters to the noise
    representation used by the ensemble optimiser.

    The noise parameterisation is:
        noise[0] = (alpha_x - xalpha) / 1.5
        noise[1] = log(beta_x  / xbeta)  / 0.8
        noise[2] = log(emitt_x / xnemit) / 0.3
        (and analogously for y)

    The returned noise vector has the property that
        noise2cs(cs2noise(cs_prior, *cs_ref), *cs_ref) == cs_prior

    This is used by cs_reconstruct() to build the prior-anchoring
    regularisation loss entirely in the bounded noise space.

    Parameters
    ----------
    cs : torch.Tensor, shape (6,) or (N, 6)
        Courant-Snyder parameters [alpha_x, beta_x, emitt_x,
                                    alpha_y, beta_y, emitt_y].
    xalpha … ynemit : float
        Reference CS values (the centre of the noise parameterisation).
        These should match the cs_ref used by the EnvelopeEnsembleModel.

    Returns
    -------
    noise : torch.Tensor, same batch shape as input, shape (..., 6)
    """
    if cs.dim() == 1:
        cs = cs.unsqueeze(0)
        squeeze_back = True
    ...
    inv_weights = torch.tensor([1.0/1.5, 1.0/0.8, 1.0/0.3,
                                 1.0/1.5, 1.0/0.8, 1.0/0.3], device=cs.device, dtype=cs.dtype)
    refs   = torch.tensor([xalpha, xbeta, xnemit, yalpha, ybeta, ynemit],
                           device=cs.device, dtype=cs.dtype)

    # alpha/yalpha: linear shift
    # beta/ybeta, emitt/ynemit: log-ratio
    n0 = (cs[:, 0] - refs[0]) * inv_weights[0]
    n1 = torch.log(cs[:, 1] / refs[1]) * inv_weights[1]
    n2 = torch.log(cs[:, 2] / refs[2]) * inv_weights[2]
    n3 = (cs[:, 3] - refs[3]) * inv_weights[3]
    n4 = torch.log(cs[:, 4] / refs[4]) * inv_weights[4]
    n5 = torch.log(cs[:, 5] / refs[5]) * inv_weights[5]

    noise = torch.stack([n0, n1, n2, n3, n4, n5], dim=1)
    return noise.squeeze(0) if squeeze_back else noise
    ...
    def cs_reconstruct(self, 
        ...
        if cs_prior is not None:
            if not isinstance(cs_prior, torch.Tensor):
                ...
                cs_prior = cs_prior.to(self.dtype)
            if cs_prior.shape != (6,):
                raise ValueError(f"cs_prior must have shape (6,), got {cs_prior.shape}")
            # Validate physical parameters: beta_x, emitt_x, beta_y, emitt_y > 0
            if cs_prior[1] <= 0 or cs_prior[2] <= 0 or cs_prior[4] <= 0 or cs_prior[5] <= 0:
                raise ValueError("cs_prior beta and emittance values must be positive.")
            # Update the reference CS so that noise=0 => cs_prior
            self.cs_ref = cs_prior.clone()
            # In noise space, the prior is exactly at the origin
            cs_prior_noise = torch.zeros(1, 6, dtype=self.dtype)
        else:
            cs_prior_noise = None
        # ─────────────────────────────────────────────────────────────────────
        ...
        loss_weights = {'fitloss_bpmQ':BPMQ_weight, 
                        ...
                        'regloss_prior': prior_weight,   # 0.0 → no prior; >0 → anchored
                        }
...
class BPMQscan:
    ...
    def __post_init__(self):
        ...
        # Each entry: {'lB2': list[float], 'reason': str,
        #              'is_beamloss': bool, 'is_BPMQ_too_large': bool}
        # Populated by evaluate_candidate(); exposed via get_data() and saved
        # to pickle by save_data() so the GUI can show rejection details after
        # a file reload.
        self.rejection_log: list = []

        # Selection history — one bool[n_scan] array per GUI-driven partial
        # retrain (train_model(selection_mask=...)).  Scans excluded by the
        # user are False.  Appended by train_model(); exposed via get_data().
        self.train_data_selection_history: list = []
        ...
        self.cs_ensemble_history      = []   # List[np.ndarray (B,6)] per train call
        self.discriminability_history = []   # List[float] per query_candidate call
        self.fit_residuals_history    = []   # List[np.ndarray (n_scan,n_bpm)]
        self.log_det_cs_history       = []   # List[float]
        self.mmd4d_change_history     = []   # List[float]
    ...
    def get_data(self):
        data = {
            ...
            'cs_ensemble_history'      : self.cs_ensemble_history,
            'discriminability_history' : self.discriminability_history,
            'fit_residuals_history'    : self.fit_residuals_history,
            'log_det_cs_history'       : self.log_det_cs_history,
            'mmd4d_change_history'     : self.mmd4d_change_history,
            # ── GUI / data-quality additions ──────────────────────────────
            # rejection_log: list of dicts, one per rejected quad setting.
            # Each entry: {lB2, reason, is_beamloss, is_BPMQ_too_large}
            'rejection_log'                : list(self.rejection_log),
            # train_data_selection_history: list of bool[n_scan] arrays,
            # one per GUI-driven partial retrain (train_model(selection_mask=...)).
            'train_data_selection_history' : list(self.train_data_selection_history),
        }
        return data
    ...
    def initialize(self,lB2=None, init_llB2=None, n_init=None, cs_prior=None, prior_weight=0.0):
        '''
        ...
        if init_llB2 is None:
            ...
            # ── First data point: always a passive read, never set quads ─────
            # For real machine (machineIO is not None): the df was already
            # fetched at the top of initialize() via quads_evaluator.read()
            # and stored as self.init_status.  Reuse it here so we never send
            # a caput for the first point — avoiding any set/readback
            # discrepancy and ensuring we never perturb the current setting.
            # This applies for ALL n_init values (single-shot or multi-shot).
            # For virtual machine (machineIO is None): evaluate_candidate runs
            # a pure simulation so there is no set/readback concern; call it
            # normally.
            print("First data point (passive read at current operating point)")
            if self.machineIO is not None:
                df_first = self.init_status
                lBPMQ_first = df_first[[name+':beamQ' for name in self.BPM_names]].mean()
                if 'GP' in self.BPMQ_model_type:
                    lBPMQtol_first      = df_first[[name+':beamQ_model_err' for name in self.BPM_names]].mean()
                    lBPMQmodelerr_first = lBPMQtol_first
                else:
                    lBPMQtol_first = lBPMQmodelerr_first = None
                is_BPMQ_too_large_first = np.any(np.abs(lBPMQ_first.values) > 40)
                if n_init == 1:
                    # ── Single-shot mode: never change quadrupole settings. ──────────
                    # Accept the passive read unconditionally, regardless of whether
                    # max|BPMQ| exceeds the threshold.  Because this point is always
                    # accepted, len(train_llB2) == 1 >= n_init after this block, so
                    # the subsequent random-sample loop breaks immediately without
                    # ever calling evaluate_candidate() — guaranteeing no quad writes.
                    if is_BPMQ_too_large_first:
                        print("[Warning] First point: |BPMQ| > 40 mm² — accepted anyway (n_init=1, no-scan mode).")
                    lB2_t           = torch.tensor(lB2, dtype=self.dtype)
                    lBPMQ_t         = torch.tensor(lBPMQ_first.values, dtype=self.dtype)
                    lBPMQtol_t      = (torch.tensor(lBPMQtol_first.values, dtype=self.dtype)
                                       if lBPMQtol_first is not None else None)
                    lBPMQmodelerr_t = (torch.tensor(lBPMQmodelerr_first.values, dtype=self.dtype)
                                       if lBPMQmodelerr_first is not None else None)
                    self.evaluated_dfs.append(df_first)
                    self._concat_BPMQ_train_data(lB2_t, lBPMQ_t, lBPMQtol_t, lBPMQmodelerr_t)
                    print("First (and only) data point accepted unconditionally (n_init=1).")
                elif is_BPMQ_too_large_first:
                    print("[Warning] First point: |BPMQ| > 40 mm² — skipping, added to penalty list.")
                    lB2_t = torch.tensor(lB2, dtype=self.dtype)
                    self.llB2_penal = lB2_t.unsqueeze(0) if self.llB2_penal is None \
                                      else torch.cat((self.llB2_penal, lB2_t.unsqueeze(0)), dim=0)
                    self.rejection_log.append({
                        'lB2': lB2_t.tolist(),
                        'reason': '|BPMQ| > 40 mm² (first passive read)',
                        'is_beamloss': False,
                        'is_BPMQ_too_large': True,
                    })
                else:
                    lB2_t            = torch.tensor(lB2, dtype=self.dtype)
                    lBPMQ_t          = torch.tensor(lBPMQ_first.values,  dtype=self.dtype)
                    ...
                    self.evaluated_dfs.append(df_first)
                    self._concat_BPMQ_train_data(lB2_t, lBPMQ_t, lBPMQtol_t, lBPMQmodelerr_t)
                    print("First data point accepted.")
            else:
                # Virtual machine — simulation only, no hardware involved.
                is_not_useful_data = self.evaluate_candidate(torch.tensor([lB2], dtype=self.dtype))
            # ── End first data point ──────────────────────────────────────

            # Remaining n_init-1 points via normal evaluate_candidate (sets quads).
        ...
        self.train_model(cs_prior=cs_prior, prior_weight=prior_weight)
...
#                 is_beamloss = np.any(BPM_MAGs < 0.95*self.init_BPM_MAGs)
            ...
            if is_beamloss or is_BPMQ_too_large:
                ...
                # Record structured rejection entry for GUI / post-analysis.
                _reasons = []
                if is_beamloss:       _reasons.append("Beam loss")
                if is_BPMQ_too_large: _reasons.append("|BPMQ| > 40 mm²")
                self.rejection_log.append({
                    'lB2':               lB2.detach().cpu().numpy().tolist(),
                    'reason':            " + ".join(_reasons),
                    'is_beamloss':       bool(is_beamloss),
                    'is_BPMQ_too_large': bool(is_BPMQ_too_large),
                })
            else:
                # Use .values to convert pandas Series to numpy array before
                # passing to torch.tensor, avoiding FutureWarning about
                # integer-key indexing being deprecated on Series objects.
                _lBPMQ = lBPMQ.values if hasattr(lBPMQ, 'values') else lBPMQ
                _lBPMQtol = lBPMQtol.values if (lBPMQtol is not None and hasattr(lBPMQtol, 'values')) else lBPMQtol
                _lBPMQmodelerr = lBPMQmodelerr.values if (lBPMQmodelerr is not None and hasattr(lBPMQmodelerr, 'values')) else lBPMQmodelerr
                lBPMQ = lBPMQ if isinstance(lBPMQ, torch.Tensor) else torch.tensor(_lBPMQ, dtype=self.dtype)
                lBPMQtol = torch.tensor(_lBPMQtol, dtype=self.dtype) if _lBPMQtol is not None and not isinstance(lBPMQtol, torch.Tensor) else lBPMQtol
                lBPMQmodelerr = torch.tensor(_lBPMQmodelerr, dtype=self.dtype) if _lBPMQmodelerr is not None and not isinstance(lBPMQmodelerr, torch.Tensor) else lBPMQmodelerr
    ...
    def train_model(self, train_llB2=None,train_llBPMQ=None,train_llBPMQtol=None,train_llBPMQmodelerr=None,
                          ...
                          cs_prior=None,
                          prior_weight=0.0):
        # selection_mask: optional bool array of length n_scan.
        # When provided (GUI-driven partial retrain), records which scans were
        # included so the history survives in the pickle.
        if selection_mask is not None:
            self.train_data_selection_history.append(
                np.asarray(selection_mask, dtype=bool).copy()
            )
    ...
    def _compute_fit_residuals(self):
        """BPMQ residuals (mm2) for all training points vs ensemble mean CS."""
        if self.train_llB2 is None or len(self.train_llB2) == 0:
            return None
        with torch.no_grad():
            xcovs = self.model.xcovs_mean   # (1,2,2)
            ycovs = self.model.ycovs_mean
            # Save current quad state – reconfigure_quadrupole_strengths mutates
            # the model in-place.  Leaving it on the last training point would
            # corrupt subsequent query_candidate or simulate calls.
            saved_B2 = torch.stack([
                elem.properties['B2'].clone()
                if isinstance(elem.properties['B2'], torch.Tensor)
                else torch.tensor(float(elem.properties['B2']), dtype=self.model.dtype)
                for elem in self.model.quads_to_scan
            ])
            residuals = []
            for i, lB2 in enumerate(self.train_llB2):
                self.model.reconfigure_quadrupole_strengths(lB2)
                l_xcovs, l_ycovs = self.model.simulate_beam_covars(
                    xcovs, ycovs, self.i_bpms)
                # l_xcovs shape: (n_monitors, batch=1, 2, 2)
                bpmq_sim = (l_xcovs[:, 0, 0, 0] - l_ycovs[:, 0, 0, 0]) * 1e6  # (n_bpm,)
                residuals.append((bpmq_sim - self.train_llBPMQ[i]).numpy())
            # Restore quad state
            self.model.reconfigure_quadrupole_strengths(saved_B2)
        return np.array(residuals)   # (n_scan, n_bpm)
            
    def _record_convergence_state(self):
        """Call at end of train_model() to snapshot the convergence metrics."""
        # 1. full CS ensemble
        with torch.no_grad():
            cs_ens = self.model.noise2cs(
                self.model.best_noise_ensemble[:, :6]
            ).detach().cpu().numpy()   # (B, 6)
        self.cs_ensemble_history.append(cs_ens)

        # 2. log det of 6×6 posterior covariance
        if cs_ens.shape[0] >= 7:
            cov6 = np.cov(cs_ens.T) + 1e-30 * np.eye(6)
            sign, logdet = np.linalg.slogdet(cov6)
            self.log_det_cs_history.append(float(logdet) if sign > 0 else np.nan)
        else:
            self.log_det_cs_history.append(np.nan)

        # 3. MMD4D change rate between consecutive iterations
        if len(self.reconstructed_cs_history) >= 2:
            cs_prev = np.array(self.reconstructed_cs_history[-2], dtype=float)
            cs_curr = np.array(self.reconstructed_cs_history[-1], dtype=float)
            try:
                mmd = calculate_MMD4D(cs_prev, cs_curr)
            except Exception:
                mmd = np.nan
            self.mmd4d_change_history.append(float(mmd))

        # 4. BPMQ fit residuals
        self.fit_residuals_history.append(self._compute_fit_residuals())
    ...
    def step(self, xnemit_target=None, ynemit_target=None,
             fit_err=None, sample_model_err=None, bootstrap=None,
        ...
        if not is_not_useful_data and not dont_train_model:
            # _record_state=False: step() owns the single _record_convergence_state call
            # (train_model default _record_state=True would double-record otherwise)
            self.train_model(xnemit_target=xnemit_target,
                             ynemit_target=ynemit_target,
                             fit_err=fit_err,
                             sample_model_err=sample_model_err,
                             bootstrap=bootstrap,
