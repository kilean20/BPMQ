...
class EnvelopeEnsembleModel:
    ...
    def calculate_beam_loss(apertures, xcovs, ycovs, max_sigma_level = 6):
        max_rms = (torch.max(xcovs[:,:,0,0], ycovs[:,:,0,0]).max(dim=1).values)**0.5
        return torch.relu((max_sigma_level * max_rms / apertures - 1)).max() ** 2
